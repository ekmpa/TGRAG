...
def load_degrees(slice_name: str) -> pd.DataFrame:
...
def load_pagerank_txtgz(slice_name: str) -> pd.DataFrame:
    ...
    try:
        with gzip.open(path, 'rt', encoding='utf-8') as f:
            ...
            if not lines:
                return None
            df = pd.read_csv(io.StringIO(''.join(lines)), sep="\t", header=None, names=["pagerank", "node_id"])
        return df
    except Exception as e:
        print(f"Error loading PageRank .txt.gz for {slice_name}: {e}")
        return None

def print_metadata_for_slice(slice_name: str, metadata_dict: dict):
...
def analyze_slice(slice_name: str, metadata_dict: dict):
    print(f"========== {slice_name} ==========")
    ...
    if degrees_df is not None and not degrees_df.empty:
        ...
        density = num_edges / (total_nodes * (total_nodes - 1)) if total_nodes > 1 else 0
    ...
    if pagerank_df is not None and not pagerank_df.empty:
        ...
        print(f"PageRank Stats: min={pagerank_df['pagerank'].min():.6f}, max={pagerank_df['pagerank'].max():.6f}, mean={pagerank_df['pagerank'].mean():.6f}")
    else:
...
def main():
    ...
    slice_names = sorted([
        d for d in os.listdir(slices_dir)
